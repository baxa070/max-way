...
def get_product_by_id(product_id):
    with closing(connection.cursor()) as cursor:
        cursor.execute("""select id, title as name, description, image, price from maxway_product where id = %s""",
                       [product_id])
...
def get_order_max_id():
    with closing(connection.cursor()) as cursor:
        cursor.execute("""select max(id) from maxway_order""")
...
def get_product_price(pk):
    with closing(connection.cursor()) as cursor:
        cursor.execute("""select price from maxway_product where id = %s""", [pk])
